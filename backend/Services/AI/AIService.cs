    ...
    public class AIService : IAIService {
        ...
        public async Task<string> SendMessageAsync(string message, ConversationContext context)
        {
            ...
            try
            {
                ...
                {
                    _logger.LogError("Google returned error {StatusCode}: {Body}", 
                                    response.StatusCode, responseString);
                    // or throw a custom exception with that info
                    throw new HttpRequestException(response.ReasonPhrase ?? "Error");
                }
